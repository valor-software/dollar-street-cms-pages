angular.module('job')
  .controller('CreateAndEditPlaceController', ['$scope', '$modalInstance', '$http', 'object', 'title', 'isAdmin', 'cmsConfig',
    function ($scope, $modalInstance, $http, object, title, isAdmin, cmsConfig) {
      function initVariable(object) {
        ...
        $scope.photographer = {};
        $scope.incomeQualityRating = [{value: 1}, {value: 2}, {value: 3}, {value: 4}, {value: 5}, {value: 6}, {value: 7}, {value: 8}, {value: 9}, {value: 10}];
      ...
      if (object.editPlaces) {
        ...
        $scope.list = $scope.isAdmin ? 'white' : 'black';
        $scope.rating = 3;
      }
      ...
      $scope.submitPlace = function (name, description, country, type, photographer, income, incomeQuality, rating, list, aboutData) {
        ...
        if (name && country && type && photographer && isNumeric(income) && incomeQuality && $scope.iconNameTrue) {
          if (!object.editPlaces) {
            $http.post(cmsConfig.serverApi + '/places/new', {
              name: name,
            ...
            $http.post(cmsConfig.serverApi + '/places/edit/' + $scope.editPlaces._id, updatePlace).success(function (data) {
              if (data.error) {
                console.error(data.error);
                return;
              }
