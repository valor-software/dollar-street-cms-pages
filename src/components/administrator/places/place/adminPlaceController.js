angular.module('job')
    ...
    function (async, _, $scope, $http, $stateParams, $sce, $window, mediaObj,
      ...
      $scope.openModalMedia = function () {
        var modalInstance = $modal.open({
          ...
          controller: ['$scope', '$modalInstance', 'placeId',
            function ($scope, $modalInstance, placeId) {
              $scope.placeId = placeId;
              // serverUrl
              $scope.videoUrl = cmsConfig.serverApi + '/upload/' + $scope.placeId + '/video';
              $scope.imageUrl = cmsConfig.serverApi + '/upload/' + $scope.placeId + '/image';
      ...
      $scope.saveTable2 = function (form) {
        ...
        _.each(infos, function (info) {
          _.each($scope.place.info, function (placeInfo) {
            if (info.id === placeInfo.id) {
              _.each(info.forms, function (form) {
                _.each(placeInfo.forms, function (placeInfoForm) {
                  if (form.formId === placeInfoForm.formId) {
                    ...
                    var indexF = placeInfo.forms.map(function (e) {
                      return e.formId;
                    }).indexOf(form.formId);
              ...
              var indexQ = $scope.place.info.map(function (e) {
                return e.id;
              }).indexOf(info.id);
        ...
        $http.post(cmsConfig.serverApi + '/places/editInfo/' + $scope.place._id, {info: infos}).success(function (data) {
          if (data.error) {
          ...
          if (el) {
            ...
            if (image) {
              ...
              $http.post(cmsConfig.serverApi + '/placeInfoImageUpload/' + $scope.placeId, fd, {
      ...
      $scope.checkedSelectForm = function (form) {
        if (form) {
          ...
          $scope.questions = _.filter($scope.cloneQuestions, function (question) {
            var pos = question.forms.map(function (e) {
              return e._id;
            }).indexOf(form._id);
      ...
      $scope.saveFamilyInfo = function () {
        $http.post(cmsConfig.serverApi + '/place/editFamilyInfo/' + $scope.place._id, {
  ...
  .service('mediaObj', [function () {
    return {countProg: 0, type: 'warning', closeProgres: false};
  }])
  .service('addNewThing', ['$http', function ($http) {
    return {
